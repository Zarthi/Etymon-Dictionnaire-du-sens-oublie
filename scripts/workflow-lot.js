...
async function chaine(d) {
  ...
  const ajouts = [...premiere.ajouts, ...(reprise?.ajouts ?? [])];
  const references = [...premiere.references, ...(reprise?.references ?? [])];
  if (verdict?.decision !== "accepte") return { ...d, notes, ajouts, aPart: "refusée deux fois par la relecture critique", remarques: verdict?.remarques ?? [] };
  return { ...d, notes, ajouts, references, reprise: reprise !== null };
}
...
return {
  ...
  // Langues et traditions ajoutées, thèmes proposés : à noter au journal, et à trancher entre deux lots.
  ajouts: [...traites.flatMap((t) => (t.ajouts ?? []).map((a) => ({ mot: t.mot, ...a }))), ...lectures.filter(Boolean).flatMap((l) => l.ajouts.map((a) => ({ mot: l.mot, ...a })))],

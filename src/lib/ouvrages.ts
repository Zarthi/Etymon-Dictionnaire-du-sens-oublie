 ...
 * `{entree}` est remplacé par l'entrée, `{grec}` par sa translittération (Bailly).
 */
...
/** Adresse d'une entrée, déduite de l'ouvrage et de l'entrée (« Littré », « étonner » ; « Bailly », « φρήν »). */
export function urlDeduite(ouvrage: string, entree: string): string | undefined {
  return modeles
    .get(ouvrage)
    ?.replace("{entree}", encodeURIComponent(entree))
    .replace("{grec}", encodeURIComponent(translitterer(entree)));
}
